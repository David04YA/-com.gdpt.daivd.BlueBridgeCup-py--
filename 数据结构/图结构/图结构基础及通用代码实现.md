...
[可视化面板](https://labuladong.online/algo/intro/visualize/) 支持创建图结构，你可以打开下面的可视化面板，即可看到图的逻辑结构，以及邻接表和邻接矩阵的存储方式：

[图结构的可视化](https://labuladong.online/algo/data-structure-basic/graph-basic/#div_graph-example)

图结构可以对很多复杂的问题进行抽象，产生了很多经典的图论算法，比如 [二分图算法](https://labuladong.online/algo/data-structure/bipartite-graph/)、[拓扑排序](https://labuladong.online/algo/data-structure/topological-sort/)、[最短路径算法](https://labuladong.online/algo/data-structure/dijkstra/)、[最小生成树算法](https://labuladong.online/algo/data-structure/kruskal/) 等，这些都会在后文介绍。

本文主要介绍图的基本概念，以及如何用代码实现图结构。

## [图的逻辑结构](https://labuladong.online/algo/data-structure-basic/graph-basic/#图的逻辑结构)

一幅图是由**节点 (Vertex)** 和**边 (Edge)** 构成的，逻辑结构如下：

![0](C:\Users\26705\Desktop\IDE\TsingPig蓝桥杯py速成\数据结构\图结构\0.jpg)
...
## [邻接表和邻接矩阵实现图结构](https://labuladong.online/algo/data-structure-basic/graph-basic/#邻接表和邻接矩阵实现图结构)

邻接表和邻接矩阵是图结构的两种实现方法。

比如还是这幅图![0](C:\Users\26705\Desktop\IDE\TsingPig蓝桥杯py速成\数据结构\图结构\0.jpg)

用邻接表和邻接矩阵的存储方式分别如下：

![image-20251107082802696](C:\Users\26705\AppData\Roaming\Typora\typora-user-images\image-20251107082802696.png)
...
## [不同种类的图结构](https://labuladong.online/algo/data-structure-basic/graph-basic/#不同种类的图结构)
...
![image-20251107101548548](C:\Users\26705\AppData\Roaming\Typora\typora-user-images\image-20251107101548548.png)
...
## [图结构的通用代码实现](https://labuladong.online/algo/data-structure-basic/graph-basic/#图结构的通用代码实现)
...
### [有向加权图（邻接表实现）](https://labuladong.online/algo/data-structure-basic/graph-basic/#有向加权图-邻接表实现)
...
### [有向无权图（邻接表/邻接矩阵实现）](https://labuladong.online/algo/data-structure-basic/graph-basic/#有向无权图-邻接表-邻接矩阵实现)

直接复用上面的 `WeightedDigraph` 类就行，把 `addEdge` 方法的权重参数默认设置为 1 就行了。比较简单，我就不写代码了。

### [无向加权图（邻接表/邻接矩阵实现）](https://labuladong.online/algo/data-structure-basic/graph-basic/#无向加权图-邻接表-邻接矩阵实现)
...
### [无向无权图（邻接表/邻接矩阵实现）](https://labuladong.online/algo/data-structure-basic/graph-basic/#无向无权图-邻接表-邻接矩阵实现)
