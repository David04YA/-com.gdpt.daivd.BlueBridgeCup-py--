...
> **示例 1：**
>
> ![img](https://labuladong.online/algo/images/lc/uploads/2021/03/03/pal1linked-list.jpg)
>
> ```
> 输入：head = [1,2,2,1]
> 输出：true
...
> **示例 2：**
>
> ![img](https://labuladong.online/algo/images/lc/uploads/2021/03/03/pal2linked-list.jpg)
>
> ```
> 输入：head = [1,2]
> 输出：false
...
class Solution:
    ...
    def traverse(self, right: ListNode):
        ...
        # 最深层的函数 (traverse(最后的节点1)) 先执行完自己的比较逻辑，然后结束。
# 它的结束，让上一层的函数 (traverse(节点2)) 得以继续执行，并用它自己的 right (也就是倒数第二个节点) 进行比较，然后它也结束。
...
你可以点开下面这个可视化面板，多次点击`if(right ===null)`这一行代码，即可看到 `right` 指针利用递归堆栈走到了链表尾部，然后再多次点击 这一行代码，即可看到 `left` 前进，`right` 指针回退，相向而行，最终完成回文判断：

[Java实现判断回文链表](https://labuladong.online/algo/data-structure/palindrome-linked-list/#div_is-palindrome)
...
![快慢指针](C:\Users\26705\Desktop\IDE\TsingPig蓝桥杯py速成\算法做题思维\快慢指针.jpg)
...
![从中点判断回文链表](C:\Users\26705\Desktop\IDE\TsingPig蓝桥杯py速成\算法做题思维\从中点判断回文链表.jpg)
...
class Solution:
    def isPalindrome(self, head: ListNode) -> bool:
        ...
        while fast and fast.next:
            slow = slow.next
            fast = fast.next.next
        #如果链表长度为奇数，fast 不为 null，slow 需要再往后移一位
        if fast:
            slow = slow.next
        
        left = head
        right = self.reverse(slow) # 从 slow 开始反转后面的链表
        while right:
            if left.val != right.val:
                return False
            #移动指针继续比较下个节点
        ...
        return True
        # 反转链表 pre前驱节点 cur当前节点 next后继节点
    def reverse(self, head: ListNode) -> ListNode:
        pre = None
        cur = head #指向要反转的链表头节点
        while cur:
            next = cur.next # 先把 cur . next 暂存起来
            cur.next = pre  #将当前节点的Next指向None
            pre = cur #前驱指针移动到当前节点
            cur = next # 当前指针移动到下一个节点
        return pre
```

你可以点开下面的可视化面板，多次点击`while(right != null)`这一行代码，即可看到 `left` 和 `right` 指针相向而行，最终完成回文判断：

[Java实现回文链表的判断](https://labuladong.online/algo/data-structure/palindrome-linked-list/#div_palindrome-linked-list)

算法总体的时间复杂度 O(N)，空间复杂度 O(1)，已经是最优的了。

我知道肯定有读者会问：这种解法虽然高效，但破坏了输入链表的原始结构，能不能避免这个瑕疵呢？

其实这个问题很好解决，关键在于得到`p, q`这两个指针位置：![pq指针的获取](C:\Users\26705\Desktop\IDE\TsingPig蓝桥杯py速成\算法做题思维\pq指针的获取.jpg)
